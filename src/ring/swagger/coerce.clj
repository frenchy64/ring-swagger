...
(defmulti time-matcher (if (System/getProperty "ring.swagger.coerce.identity-time-matcher-dispatch")
                         identity
                         #(when (class? %) %)))
...
(defmulti custom-matcher (if (System/getProperty "ring.swagger.coerce.identity-custom-matcher-dispatch")
                           identity
                           #(when (class? %) %)))
(defmethod custom-matcher :default [_] nil)

(defmulti coercer (if (System/getProperty "ring.swagger.coerce.identity-coercer-dispatch")
                    identity
                    #(when (keyword? %) %)))
